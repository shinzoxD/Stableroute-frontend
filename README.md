...
- **`/api-keys`** ([api-keys/page.tsx](src/app/api-keys/page.tsx)): Dashboard to create, list, and revoke API keys. Create (`POST /api/v1/api-keys`) returns the full secret once; the UI surfaces it in a `role="status"` “Copy now — shown only once” block (secure context only), then only the prefix is kept in list state. Revoke calls `DELETE /api/v1/api-keys/:prefix`. Newly created keys are marked with a "New" badge and their creation time is displayed via relative timestamps (`TimeAgo`).
